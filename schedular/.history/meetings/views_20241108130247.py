...
class AvailabilityView(APIView):
    ...
    def get(self, request):
        ...
        for availability in availabilities:
            # Create a UTC datetime using a reference date
            reference_date = datetime(2000, 1, 1)  # Use any fixed reference date
            
            # Create UTC datetimes directly since the times in database are already in UTC
            start_time_utc = datetime.combine(reference_date, availability.start_time)
            end_time_utc = datetime.combine(reference_date, availability.end_time)
