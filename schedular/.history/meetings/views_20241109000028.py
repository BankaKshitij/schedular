...
from .models import Availability, Meeting, MeetingAttendee, MeetingEditHistory, MeetingExtensionHistory, MeetingType
...
class ScheduleMeetingView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Convert start and end times from UTC to organizer's timezone for calculating day_of_week
    ...
    def patch(self, request, id):
        ...
        try:
            ...
            if next_meeting:
                ...
                with transaction.atomic():
                    # Save extension history before updating meeting details
                    MeetingExtensionHistory.objects.create(
                        meeting=meeting,
...
class RescheduleSuggestionView(APIView):
    ...
    def post(self, request, meeting_id):
        ...
        if not suggestions:
            return Response({"detail": "Could not get suggestions from GPT"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        return Response(suggestions, status=status.HTTP_200_OK)

    def format_prompt(self, meeting, all_availability):
        prompt = f"""Suggest suitable time slots to reschedule the following meeting:
...
"""
        ...
        for username, data in all_availability.items():
            prompt += f"User: {username} (Timezone: {data['timezone']})\n"
            for slot in data['availability_slots']:
                prompt += f"    Day: {slot['day_of_week']}, From: {slot['start_time']} to {slot['end_time']}\n"

        prompt += """
Please suggest 3 suitable time slots that are available for all participants.
Provide the suggested start and end times in ISO format (YYYY-MM-DDTHH:MM) and make sure they work for the time zones of all participants.
Also, include a reschedule API cURL request for each suggestion so it can be directly used to reschedule the meeting.
"""

        return prompt

    def get_gpt_suggestions(self, prompt):
        # Here you call the OpenAI GPT API
        headers = {
        ...
        if response.status_code == 200:
            ...
            suggestions_text = gpt_response.get("choices")[0].get("text")
    ...
    def parse_gpt_suggestions(self, suggestions_text):
        ...
        for line in lines:
            line = line.strip()
            if line.startswith("Suggested Slot"):
                if current_suggestion:
                    suggestions.append(current_suggestion)
                    current_suggestion = {}

            elif "Start Time:" in line and "End Time:" in line:
                parts = line.split(", ")
                current_suggestion['new_start_time'] = parts[0].replace("Start Time: ", "").strip()
                current_suggestion['new_end_time'] = parts[1].replace("End Time: ", "").strip()

            elif line.startswith("cURL:"):
                current_suggestion['cURL'] = line.replace("cURL: ", "").strip()
