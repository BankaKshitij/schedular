...
class RescheduleSuggestionView(APIView):
    ...
    def post(self, request, meeting_id):
        ...
        for user_id in users_to_check:
            ...
            # Retrieve existing meetings for the user
        ...
        # Format prompt and get suggestions
        prompt = self.format_prompt(meeting, all_availability)
        suggestions = self.get_claude_suggestions(prompt)
    ...
    def format_prompt(self, meeting, all_availability):
        prompt = f"""You are a scheduling assistant. Please analyze the following meeting information and participant availability to suggest optimal meeting times.

Meeting Details:
Title: {meeting.title}
Current Start: {meeting.start_time.strftime("%Y-%m-%d %H:%M")}
Current End: {meeting.end_time.strftime("%Y-%m-%d %H:%M")}
...
"""
        ...
        prompt += """\nPlease suggest 3 suitable meeting times that work for all participants. For each suggestion, provide:
1. Start time in ISO format (YYYY-MM-DDTHH:MM)
2. End time in ISO format (YYYY-MM-DDTHH:MM)
3. A brief explanation of why this time works
4. The API request needed to reschedule
...
API Request: [curl command]"""

        return prompt

    def get_claude_suggestions(self, prompt):
        ...
        try:
            ...
            return self.parse_suggestions(suggestions_text)
        except Exception as e:
        ...
        try:
            ...
            for block in suggestion_blocks:
                ...
                for line in lines:
                    ...
                    elif "API Request:" in line:
                        current_suggestion['api_request'] = line.split("API Request:")[1].strip()
